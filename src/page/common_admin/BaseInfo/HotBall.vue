	...
	<div>
		...
		<el-table :data="allData" highlight-current-row v-loading="listLoading"  style="width: 100%;" max-height="500">
			...
			<el-table-column align='center' prop="" :label="$route.name+'图片'">
				<template scope='scope'> 
					<DialogCarousel :pictures='scope.row.noticeBoard.notiBalloonNotice? scope.row.noticeBoard.notiSchedule.split(",") : []'></DialogCarousel>
				</template>	
			</el-table-column>
		...
		ref="addFormDialog">
			...
			<el-form :model="addFormSendData" label-width="150px" ref="addForm" >
				...
				<el-form-item :label="$route.name+'名称'">
					<el-input v-model="addFormSendData.noticeBoard.notiBalloonNotice" :placeholder="$route.name+'名称'"></el-input>
				</el-form-item>
	...
	export default{
		...
		methods:{
			...
			submitForm(form){
				...
				this[_params]['noticeBoard']['notiBalloonNotice'] = this.arrToStr(this.imgsArr) || this.arrToStr(this.existImgList)					
